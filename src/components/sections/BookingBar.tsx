...
export function BookingBar({
  ...
  const { register, handleSubmit, watch, formState: { errors } } = useForm<QuickBookingValues>({
  ...
  const errorMessage = errors.checkIn
    ? t.booking.pastDate
    : errors.checkOut
      ? t.booking.dateOrder
  ...
  return (
    <section className="sticky top-[72px] z-30 hidden px-4 md:block" aria-label={t.booking.title}>
      <form onSubmit={handleSubmit(onSubmit)} className="mx-auto grid max-w-7xl grid-cols-[repeat(5,minmax(0,1fr))_auto] gap-2 rounded-lg border border-white/35 bg-navy/82 p-3 shadow-glow backdrop-blur-xl">
        <input aria-label={t.booking.checkIn} className={input} type="date" min={today} {...register("checkIn")} />
        <input aria-label={t.booking.checkOut} className={input} type="date" min={watch("checkIn") || today} {...register("checkOut")} />
        <input aria-label={t.booking.guests} className={input} type="number" min={1} max={30} {...register("guests")} />
