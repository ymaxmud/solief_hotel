...
// `contact` still supplies the fixed map embed / directions URLs and the
// district label — design content rather than owner-editable settings.
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid gap-6 md:grid-cols-[.9fr_1.1fr]">
          <div className="rounded-2xl border border-line bg-white p-6 shadow-soft">
            ...
            <h3 className="mt-4 font-display text-3xl text-ink">{site.address}</h3>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <ButtonLink href={site.googleMapsUrl} target="_blank"><MapPin size={17} /> {t.actions.map}</ButtonLink>
              <ButtonLink href={contact.directionsUrl} target="_blank" variant="secondary"><Navigation size={17} /> {t.actions.directions}</ButtonLink>
              <ButtonLink href={site.googleReviewsUrl || site.googleMapsUrl} target="_blank" variant="light"><ExternalLink size={17} /> {t.actions.googleProfile}</ButtonLink>
              <ButtonLink href={`tel:${site.phoneE164}`} variant="light"><Phone size={17} /> {t.actions.call}</ButtonLink>
            </div>
          </div>
          <div className="overflow-hidden rounded-2xl border border-line bg-slate shadow-glow">
            <div className="flex items-center justify-between gap-4 border-b border-white/10 bg-navy px-5 py-4 text-white">
              ...
              <a href={site.googleReviewsUrl || site.googleMapsUrl} target="_blank" rel="noopener noreferrer" className="focus-ring rounded-full bg-white/10 p-3 text-white transition hover:bg-white/20" aria-label={t.actions.googleProfile}>
