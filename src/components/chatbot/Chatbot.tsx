...
export function Chatbot({ t, locale, onBook }: { t: Dictionary; locale: Locale; onBook: () => void }) {
  ...
  return (
    <div className="fixed bottom-24 right-4 z-50 md:bottom-6">
      {open ? (
        <div className="mb-3 w-[min(92vw,360px)] overflow-hidden rounded-2xl border border-line bg-canvas shadow-glow">
          ...
          <div className="max-h-72 space-y-3 overflow-y-auto p-4" aria-live="polite">
            {messages.map((message, index) => (
              <div key={index} className={`rounded-lg p-3 text-sm ${message.from === "bot" ? "bg-white text-slate" : "ml-8 bg-slate text-white"}`}>{message.text}</div>
            ))}
          </div>
      ...
      <button className="focus-ring ml-auto flex h-14 w-14 items-center justify-center rounded-full bg-oxford text-white shadow-glow" onClick={() => setOpen(!open)} aria-label={t.chatbot.title}>
        <MessageCircle />
      </button>
    </div>
  );
